...
An example of the service is running on CloudBees at gasp-mongo.mqprichard.cloudbees.net using the integrated MongoDB service from MongoHQ, or it can be run using a local MongoDB.
This is a Maven project, written in Java using the Google Geocoding API v3, MongoDB Geospatial Indexing and JAX-RS/JAX-B for the REST service implementation.  Please note the [usage limits and conditions of use](https://developers.google.com/maps/documentation/geocoding/#Limits) for the Google Geocoding API.  The project currently uses the MongoDB legacy 2D co-ordinate system: this is perfectly accurate and adequate for its intended use, but I will be updating to use the Mongo 2.4 geospatial query operators. 
...
For geospatial queries, the units for the searh radius are the same as those for the lat/lng co-ordinates (degrees): At the latitude of the San Francisco Bay area, a value of 0.005 will give a radius of about 0.3 miles, as shown in the example.
...
Remove a geocoded location (from MongoDB collection)
----------------------------------------------------
